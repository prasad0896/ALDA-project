...
while(cost <= 10) {
  ...
  preds <- NULL
  while(ctr < 210) {
    train.df <- data.df[1:(52+ctr),]
    # print(1:(75+ctr))
    test.df <- data.df[(52+ctr+1):(52+ctr+52),]
...
points(x = log2(0.125), y = scale(new.res$AUC)[19], col = "blue")
points(x = log2(0.125), y = scale(new.res$Accuracy)[19], col = "blue")
...
plot_ly(res.df, x = ~Cost, y = ~Gamma, z = ~Accuracy, marker = list(color = ~AUC, colorscale = c('#FFE1A1', '#683531'), showscale = TRUE)) %>%
  ...
  layout(scene = list(xaxis = list(title = 'Cost'),
                      ...
                      zaxis = list(title = 'Accuracy')))
new.res$factor <- 1
new.res$factor[455] <- 2
plot_ly(new.res, x = ~Cost, y = ~Gamma, z = ~Accuracy, marker = list(color = ~factor, colorscale = c('#FFE1A1', '#683531'), showscale = TRUE)) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'Cost'),
                      yaxis = list(title = 'Gamma'),
                      zaxis = list(title = 'Accuracy')))
plot_ly(new.res, x = ~Cost, y = ~Gamma, z = ~AUC, marker = list(color = ~factor, colorscale = c('#FFE1A1', '#683531'), showscale = TRUE)) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'Cost'),
                      yaxis = list(title = 'Gamma'),
                      zaxis = list(title = 'AUC')))
...
print("TSCV Accuracy for SVM - Polynomial Kernel")
...
while(degree <= 5) {
  ...
  while(gamma <= 3) {
    ...
    while(cost <= 6) {
      ...
      res.df[i,2] <- 2^gamma
      res.df[i,3] <- degree
      res.df[i,4] <- mean(actuals == preds)
      roc_obj <- roc(actuals, preds)
      res.df[i,5] <- auc(roc_obj)
      cost <- cost + 1
      i <- i + 1
    }
    gamma <- gamma + 1
...
new.res <- res.df[order(AUC),]
detach(res.df)
write.csv(new.res, file="SVMHyperParPoly.csv", row.names = FALSE)
# plot_ly(res.df, x = ~Cost, y = ~Gamma, z = ~Accuracy, marker = list(color = ~Accuracy, colorscale = c('#FFE1A1', '#683531'), showscale = TRUE)) %>%
#   add_markers() %>%
#   layout(scene = list(xaxis = list(title = 'Cost'),
#                       yaxis = list(title = 'Gamma'),
#                       zaxis = list(title = 'Accuracy')))
plot_ly(res.df, x = ~Cost, y = ~Gamma, z = ~AUC, marker = list(color = ~factor(Degree), colorscale = c('#FFE1A1', '#683531'), showscale = TRUE)) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'Cost'),
                      yaxis = list(title = 'Gamma'),
                      zaxis = list(title = 'AUC')))
...
# Train:Test 70:30
train.df <- data.df[1:260,]
test.df <- data.df[261:374,]
...
roc.val <- roc(as.numeric(test.df[,ncol(test.df)]), as.numeric(y_pred))
print(roc.val$auc[1])
